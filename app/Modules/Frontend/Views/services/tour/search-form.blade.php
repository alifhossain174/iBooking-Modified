@php
    ...
    enqueue_styles(['mapbox-gl', 'mapbox-gl-geocoder']);
    enqueue_scripts(['mapbox-gl', 'mapbox-gl-geocoder']);
    ...
    if ($has_advanced) {
        ...
        enqueue_styles(['icon.rangeSlider']);
    
        enqueue_scripts(['icon.rangeSlider']);
    }
...
<form id="search-tour" method="GET" class="search-form tour" action="{{ url('tour-search') }}">
    <div class="search-form__basic w-75 m-auto searchh">
        ...
        @if ($has_advanced)
        @endif
        <button class="btn btn-primary search-form__search" type="submit">{{ __('Search') }}
        </button>
    </div>
                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="price-range-slider" name="price_range" value=""
                            data-min="{{ $price_range['min'] }}" data-max="{{ $price_range['max'] }}"
                            data-form="{{ $price_range['from'] }}" data-to="{{ $price_range['to'] }}"
                    ...
                    @if (!empty($tour_types))
                        <div class="col-md-6 gmz-checkbox-wrapper">
                            ...
                            @foreach ($tour_types as $key => $type)
                                <label class="checkbox-inline"><input type="checkbox" class="gmz-checkbox-item"
                                        name="tour_types[]"
                                        value="{{ $key }}"><span>{{ get_translate($type) }}</span></label>
                            @endforeach
                    ...
                    @if (!empty($tour_include))
                        <div class="col-md-6 gmz-checkbox-wrapper">
                            ...
                            @foreach ($tour_include as $key => $value)
                                <label class="checkbox-inline"><input type="checkbox" class="gmz-checkbox-item"
                                        name="tour_includes[]"
                                        value="{{ $key }}"><span>{{ get_translate($value) }}</span></label>
                            @endforeach
...
@php
    $top20 = DB::table('gmz_term')
        ->where('taxonomy_id', 16)
        ->where('term_title', 'LIKE', '%' . 'Top' . '%')
        ->first();
    $allIncVac = DB::table('gmz_term')
        ->where('taxonomy_id', 16)
        ->where('term_title', 'LIKE', '%' . 'Inclusive' . '%')
        ->first();
    $familyVac = DB::table('gmz_term')
        ->where('taxonomy_id', 16)
        ->where('term_title', 'LIKE', '%' . 'Family' . '%')
        ->first();
    $mountainVac = DB::table('gmz_term')
        ->where('taxonomy_id', 16)
        ->where('term_title', 'LIKE', '%' . 'Mountain' . '%')
        ->first();
    $adultVac = DB::table('gmz_term')
        ->where('taxonomy_id', 16)
        ->where('term_title', 'LIKE', '%' . 'Adult' . '%')
        ->first();
    $singleVac = DB::table('gmz_term')
        ->where('taxonomy_id', 16)
        ->where('term_title', 'LIKE', '%' . 'Single' . '%')
        ->first();
@endphp
            ...
            <div class="m-1 p-2 rounded" style="background-color: #df1995; min-height: 50px">
                <a href="tour-search?lat=&lng=&address=&price_range=&tour_types[]={{ $top20->id }}&tour_type={{ $top20->id }}&tour_include=&tour_exclude=&layout=grid"
                    ...
                    <div class="text-white d-flex align-items-center">
                        <div class="m-1">
                            <img class="filter_btn_img" src="{{ url('images') }}/ic_top20.png">
                        </div>
        ...
        <div class="col-lg-2 col-md-3 col-6">
            <div class="m-1 p-2 rounded" style="background-color: #df1995; min-height: 50px">
                <a href="tour-search?lat=&lng=&address=&price_range=&tour_types[]={{ $allIncVac->id }}&tour_type={{ $allIncVac->id }}&tour_include=&tour_exclude=&layout=grid"
                    ...
                    <div class="text-white d-flex align-items-center">
                        <div class="m-1">
                            <img class="filter_btn_img" src="{{ url('images') }}/ic_inclusive.png">
                        </div>
                        <div style="font-weight: 700">
                            Inclusives Vacations
                        </div>
                    </div>
                </a>
            </div>
        ...
        <div class="col-lg-2 col-md-3 col-6">
            <div class="m-1 p-2 rounded" style="background-color: #df1995; min-height: 50px">
                <a href="tour-search?lat=&lng=&address=&price_range=&tour_types[]={{ $familyVac->id }}&tour_type={{ $familyVac->id }}&tour_include=&tour_exclude=&layout=grid"
                    ...
                    <div class="text-white d-flex align-items-center">
                        <div class="m-1">
                            <img class="filter_btn_img" src="{{ url('images') }}/ic_family.png">
                        </div>
                        <div style="font-weight: 700">
                            Family Vacations
                        </div>
        ...
        <div class="col-lg-2 col-md-3 col-6">
            <div class="m-1 p-2 rounded" style="background-color: #df1995; min-height: 50px">
                <a href="tour-search?lat=&lng=&address=&price_range=&tour_types[]={{ $mountainVac->id }}&tour_type={{ $mountainVac->id }}&tour_include=&tour_exclude=&layout=grid"
                    ...
                    <div class="text-white d-flex align-items-center">
                        <div class="m-1">
                            <img class="filter_btn_img" src="{{ url('images') }}/ic_ski.png">
                        </div>
                        <div style="font-weight: 700">
                            Tracking Vacations
                        </div>
        ...
        <div class="col-lg-2 col-md-3 col-6">
            <div class="m-1 p-2 rounded" style="background-color: #df1995; min-height: 50px">
                <a href="tour-search?lat=&lng=&address=&price_range=&tour_types[]={{ $adultVac->id }}&tour_type={{ $adultVac->id }}&tour_include=&tour_exclude=&layout=grid"
                    ...
                    <div class="text-white d-flex align-items-center">
                        <div class="m-1">
                            <img class="filter_btn_img" src="{{ url('images') }}/ic_adult.png">
                        </div>
                        <div style="font-weight: 700">
        ...
        <div class="col-lg-2 col-md-3 col-6">
            <div class="m-1 p-2 rounded" style="background-color: #df1995; min-height: 50px">
                <a href="tour-search?lat=&lng=&address=&price_range=&tour_types[]={{ $singleVac->id }}&tour_type={{ $singleVac->id }}&tour_include=&tour_exclude=&layout=grid"
                    ...
                    <div class="text-white d-flex align-items-center">
                        <div class="m-1">
                            <img class="filter_btn_img" src="{{ url('images') }}/ic_singles.png">
                        </div>
                        <div style="font-weight: 700">
                            Single Vacations
                        </div>
